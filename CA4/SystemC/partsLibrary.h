...
// 16-bit Comparator
SC_MODULE(Comparator16)
{
	sc_in <sc_lv<16>> ain, bin;
	sc_out <sc_logic> lt;

	SC_CTOR(Comparator16)
	{
		SC_METHOD(compare);
		sensitive << ain << bin;
	} 
...
// 8-bit 3 to 1 multiplexer
SC_MODULE(Mux3to1_8)
{
	sc_in<sc_lv<2>> sel;
	sc_in<sc_lv<8>> ain, bin, cin;
	sc_out<sc_lv<8>> yout;
...
{
	sc_in<sc_logic> clk, rst, cen, init;
	sc_out<sc_logic> co;
	sc_in<sc_lv<8> > parin;
